...
class OnPolicyBuffer(AbstractBuffer):
    ...
    def sample(self, num_sample=-1, init_buffer=True):
        ...
        return result[:,self.OB_IDX].tolist(), result[:,self.AC_IDX].tolist(), result[:,self.AC_LOGPROB_IDX].tolist(), \
            result[:,self.REW_IDX].tolist(), result[:,self.NOB_IDX].tolist(), result[:,self.DONE_IDX].tolist(), \
                result[:,self.VALUE_IDX].tolist(), result[:,self.NEXT_VALUE_IDX].tolist()
